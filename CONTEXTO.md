...
- ✅ **Botão flutuante 💬 = portal do grupo** (ago/2026): abre `config.linkGrupo` (convite do grupo, configurado no admin ⚙️); sem link, copia um convite em texto **que inclui o endereço da vitrine quando a loja está online** (fallback monta `convite + "\n👉 " + location.href`). Etiqueta flutuante "👥 Grupo da loja!" ao lado.
